...
const requiredModules = [ // services + layout views + system views
  'compilerArtefacts', 'compilerMetadata', 'contextualListener', 'editor', 'offsetToLineColumnConverter', 'network', 'theme', 'fileManager', 'contentImport', 'scriptRunner',
...
export class RemixAppManager extends PluginEngine {
  ...
  async registeredPlugins () {
    ...
    const scriptRunner = {
      ...
      events: [],
      version: '0.1.0-beta',
      url: 'ipfs://QmWFZ4z2aHstiZL4QvFUAJcCfr9qqW3bHPsh1UF56TvoWf',
      icon: '',
      location: 'hiddenPanel'
    }
    return [new IframePlugin(scriptRunner), ...plugins.map(plugin => new IframePlugin(plugin))]
  }
}
